...
const socialIcon: Record<string, React.ComponentType<{ className?: string }>> = {
  Telegram: Send, Seguir: X, Discord: MessageCircle, Convidar: Users,
};
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between px-1">
        <h2 className="text-base font-black text-gradient-primary">Missões</h2>
        <button
          onClick={watchAd}
          className="flex items-center gap-1 rounded-xl bg-gradient-accent px-2.5 py-1.5 text-[10px] font-black text-white shadow-neon active:scale-95"
        >
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g.id}>
            <h3 className={`mb-1.5 px-1 text-[10px] font-black uppercase tracking-widest ${g.color}`}>
              {g.label}
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-xs font-black">{m.title}</div>
                      ...
                      <div className="mt-0.5 flex gap-2 text-[9px] font-bold">
                        {m.reward.gold && <span className="text-gold">+{m.reward.gold} 🪙</span>}
                        {m.reward.crystals && <span className="text-crystal">+{m.reward.crystals} 💎</span>}
                        {m.reward.ton && <span className="text-ton">+{m.reward.ton} TON</span>}
                      </div>
                    </div>
                    <button
                      onClick={() => claimMission(m.id)}
                      disabled={!complete}
                      className="shrink-0 rounded-xl bg-gradient-gold px-2.5 py-2 text-[10px] font-black text-yellow-950 shadow-neon active:scale-95 disabled:opacity-40"
                    >
